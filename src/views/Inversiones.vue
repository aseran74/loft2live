    ...
    <section class="pt-24 pb-12" style="background-color:#F2F2F2">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <div class="flex flex-col gap-2 mb-6">
          <h1 class="text-3xl sm:text-4xl font-bold" style="color:#0D0D0D">Inversiones</h1>
          <p class="text-gray-600">
            Busca oportunidades por localización y precio por loft.
          </p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow p-5 mb-6">
          <div class="flex items-center justify-between gap-4">
            <h2 class="text-lg font-bold" style="color:#0D0D0D">Búsqueda</h2>
            <button
              type="button"
          ...
          <div class="mt-4 text-sm text-gray-600">
            Mostrando <span class="font-semibold" style="color:#0D0D0D">{{ filtered.length }}</span>
            de {{ proyectos.length }} oportunidades.
          </div>
        </div>
        ...
        <!-- Lista -->
        <div v-if="loading" class="text-center py-10 text-gray-700">Cargando inversiones…</div>
        <div v-else-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6">
          {{ error }}
        </div>
        <div v-else-if="filtered.length === 0" class="text-center py-10 text-gray-700">
